...
- **`public/ifslib.wasm`** — Pure WASI reactor. Exports:
  ...
  - `init(ptr) → i32` — parse AIFS source (C string ptr). Returns the **number of blocks** found (≥1) on success, or 0 on parse error. `if (!wasm.init(ptr))` still works as a failure check.
  - `get_block_idx(block_ptr) → i32` — resolve a block string ID to its 0-based internal index **without** selecting it. Accepts block identifier, display name, or 0-based numeric string. Pass `nullptr` (0) or empty string to get the index of the first non-hidden block. Returns the index on success, or -1 if not found. Must be called after `init()`.
  - `set_block(block_idx) → i32` — select a block by its **0-based numeric index** (as returned by `get_block_idx`). Pass `-1` to select the first non-hidden block. Resets root to block default (`$root` if defined, otherwise first visible variable). Returns the **number of variables** in the block (≥1) on success, or 0 on error. Can be called multiple times to switch blocks without re-calling `init()`. **Breaking change from previous API**: now takes `int32_t` index, not `const char*` — use `get_block_idx()` to resolve a name first.
  ...
  - `get_root_name(root_idx) → charPtr` — return the variable name of the root attractor at 0-based `root_idx` within the currently selected block. Returns a null-terminated string valid until the next ifslib call, or `nullptr` if out of range or no block selected. Must be called after `set_block()`.
  ...
  - `calc_diams(max_queue_size, max_result_size) → doublePtr` — compute all geometric diameters of the root attractor. Returns an array: `[N, a1[0]..a1[DIM-1], b1[0]..b1[DIM-1], ...]` where N is the count of diameter pairs and each pair is two endpoints. Total elements: `1 + N*2*DIM`. Returns `nullptr` on error. `max_queue_size` controls search budget (larger = slower but more complete); `max_result_size` caps the number of pairs returned. Must be called after `set_block()`.
  - `calc_dists(pt_ptr, dim, max_queue_size, max_result_size) → doublePtr` — compute all points on the root attractor farthest from the given point `pt` (array of `dim` doubles in WASM memory). Returns `[N, a1[0]..a1[DIM-1], ...]`, total `1 + N*DIM` elements. Returns `nullptr` on error. Must be called after `set_block()`.
